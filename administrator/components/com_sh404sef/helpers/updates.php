...
class Sh404sefHelperUpdates
{
	...
	private static function _updateRequired($response)
	{
		...
		// compare versions
		$response->shouldUpdate = version_compare($sefConfig->version, $response->current) == -1;
		$response->shouldUpdate = $response->shouldUpdate && version_compare($sefConfig->version, $response->minVersionToUpgrade) == 1;
		$response->shouldUpdate = $response->shouldUpdate
			&& (empty($response->maxVersionToUpgrade) || version_compare($sefConfig->version, $response->maxVersionToUpgrade) == -1);
		if ($response->shouldUpdate)
		{
		...
		$response->statusMessage = $response->shouldUpdate ? JText::sprintf('COM_SH404SEF_NEW_VERSION_AVAILABLE')
