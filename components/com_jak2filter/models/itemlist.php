...
class JAK2FilterModelItemlist extends JAK2FilterModel 
{
	function getData($ordering = NULL) {
		...
		//remove check params Google search integration	
	...
	*/
	...
	function prepareSearch($search) {
		...
		//Search by keyword
		$jakeyword = JRequest::getVar('jakeyword');
		if($jakeyword){
			//Check length keyword
			if(strlen($jakeyword) >=$minium_keyword && strlen($jakeyword) <= $maximum_keyword){
				$jakeyword = '%'.$jakeyword.'%';
				$where[] = "(i.title LIKE ".$db->quote($jakeyword)." OR i.introtext LIKE ".$db->quote($jakeyword)." OR i.fulltext LIKE ".$db->quote($jakeyword).")";
			} else {
				JError::raise(E_WARNING, 100, JText::sprintf(JAK2FILTER_SEARCH_TERM,$minium_keyword,$maximum_keyword));
			}
		} else {
			$where[] = "(1)";
		}
		...
		if(JRequest::getVar('category_id')){
			...
			foreach($ja_cids AS $cid){
				...
				//{
					$wherecate[] = "i.catid = ".$db->quote($cid)." ";
				//}
				//else
				//{
		...
		//Search by Author
		$created_by = JRequest::getInt('created_by');
		if($created_by){
		...
		if(count($vars)) {
			...
			foreach ($vars as $field => $value) {
				...
				if(preg_match($exPattern, $field, $matches)) {
					$fid = $matches[1];
