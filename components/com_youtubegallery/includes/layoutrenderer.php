...
class YoutubeGalleryLayoutRenderer
{
	...
	public static function curPageURL($add_REQUEST_URI=true)
	{
		$pageURL = '';
			...
			if($add_REQUEST_URI)
			{
				...
				{
					$uri= deleteURLQueryOption($uri, 'fb_action_ids');
					$uri= deleteURLQueryOption($uri, 'fb_action_types');
					$uri= deleteURLQueryOption($uri, 'fb_source');
					$uri= deleteURLQueryOption($uri, 'action_object_map');
					$uri= deleteURLQueryOption($uri, 'action_type_map');
					$uri= deleteURLQueryOption($uri, 'action_ref_map');
				}
				$pageURL .=$uri;
			}
	...
	public static function NavigationList($the_gallery_list, &$videolist_row, &$theme_row, $AllowPagination, $videoid)
	{
        ...
        foreach($gallery_list as $listitem)	
        {
				...
				$thumbnail_item=YoutubeGalleryLayoutRenderer::renderThumbnailForNavBar($aHrefLink,$aLink,$theImage,$videolist_row, $theme_row,$listitem, $videoid);
	...
	public static function renderThumbnailForNavBar($aHrefLink,$aLink,$theImage,&$videolist_row, &$theme_row,$listitem, $videoid)
	{
		$thumbnail_layout='';
		...
		else
		{
			...
			else
			{
				...
				{
					//Put website page title if there is no title available for thumbnail
		...
		{
			$result=YoutubeGalleryLayoutRenderer::renderThumbnailLayout($theme_row->customnavlayout,$listitem,$theImage,$imagetag,$aHrefLink,$aLink, $videoid);
		}
		else
		{
			...
			$result=YoutubeGalleryLayoutRenderer::renderThumbnailLayout($thumbnail_layout,$listitem,$theImage,$imagetag,$aHrefLink,$aLink, $videoid);
		}
	...
	public static function renderThumbnailLayout($thumbnail_layout,$listitem,$theImage,$imagetag,$aHrefLink,$aLink, $videoid)
	{
		...
		$fields=array('image','link','a','/a','link','title','description',
					  ...
					  'statistics_favoriteCount','viewcount','favcount','keywords','isactive');
		...
		foreach($fields as $fld)
		{
			$isEmpty=YoutubeGalleryLayoutRenderer::isThumbnailDataEmpty($fld,$listitem,$tableFields,$theImage, $videoid);
	...
	public static function isThumbnailDataEmpty($fld,$listitem,&$tableFields,$theImage, $videoid)
	{
	...
	public static function ShowActiveVideo($gallery_list,$width,$height,$videoid, &$videolist_row, &$theme_row,$videosource='')
	{
		...
		if($videoid)
		{
			...
			else
			{
				...
				$theImage=$gallery_list[$list_index]['imageurl'];
		...
		if($videoid!='****youtubegallery-video-id****')
		{
			...
			//$result=str_replace('*/scr','</scr',$result);
	...
	public static function addHotReloadScript(&$gallery_list,$width,$height,&$videolist_row, &$theme_row)
	{
			...
			$vs=array('youtube','vimeo','break','own3dtvlive','own3dtvvideo','google','yahoo','collegehumor','dailymotion','.flv');
...
';
	...
	function YoutubeGalleryHotVideoSwitch'.$videolist_row->id.'(videoid,videosource,id)
	{
		...
		if(playercode.indexOf("<!--DYNAMIC PLAYER-->")!=-1)
			eval("youtubegallery_updateplayer_"+videosource+"_'.$videolist_row->id.'(videoid)");
